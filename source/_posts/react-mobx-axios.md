...
虽然项目已经运行，但是在项目目录里是找不到`webpack`配置项的。比如，我们想配置 less、配置 alias 别名，这时我们需要做一个操作：展开项目（`eject`）[ 注：这个一个不可逆过程，一旦你执行了，就不能回到初始化 ] 。
...
- 在`stores`里新建`index.js`、`home.js`、`about.js`。 【 注：home.js 和 about.js 我用了两种方式写，都可以的。】
  `home.js`
...
export default {
  ...
  async getList() {
    return await axios
      .get("http://lemonof.com:82/api/getList")
...
查阅文章有：
create-react-app 全家桶 router+mobx：https://www.jianshu.com/p/2d54c2b3cfa3
